...
class Tank:
    def __init__(self, x, y, color, width=TANK_WIDTH, height=TANK_HEIGHT, bullet_cooldown=BULLET_COOLDOWN, bullet_size=BULLET_SIZE):
        self.width = width
        self.height = height
    ...
    def auto_aim_shoot(self, target_tank):
    # Automatically align shooter to target tank before shooting
        dx = target_tank.x - self.x
        dy = target_tank.y - self.y
        desired_angle = (math.degrees(math.atan2(dy, dx)) + 90) % 360

        self.shooter_angle = desired_angle  # Instantly align turret
        return self.shoot()
