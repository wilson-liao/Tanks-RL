...
class TankEnv(Env):
    ...
    def step(self, action):
        ...
        reward = 0.01  # Base survival reward per step
        ...
        if action == 0:  # Move forward
            ...
            if angle_diff < 10:  # within 10 degrees tolerance
                reward += 1  # turret alignment clearly rewarded
        ...
        # Penalize for being near corners
        corner_threshold = 50
        corners = [(0, 0), (0, WINDOW_HEIGHT), (WINDOW_WIDTH, 0), (WINDOW_WIDTH, WINDOW_HEIGHT)]
        for corner_x, corner_y in corners:
            dist_to_corner = np.sqrt((self.player_tank.x - corner_threshold - corner_x)**2 + (self.player_tank.y - corner_y)**2)
            if dist_to_corner < corner_threshold:
                reward -= 5  # Penalize proximity to corner clearly
    ...
    def create_enemy_tanks(self, mode = "heuristic"):
        for i in range(NUMBER_OF_ENEMIES):
            ...
            if mode == "heuristic":
                self.enemy_tank = HeuristicBot(self, x, y, (150, 0, 0),  HEURISTIC_WIDTH, HEURISTIC_HEIGHT)
            elif mode == "random":
